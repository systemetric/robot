...
# TODO (2025): Change and fix jokes (if applicable)
...
class MARKER_TYPE(enum.Enum): # Keep something like this to determine if a marker is a wall or not.
    GAME_OBJECT = enum.auto()
    ARENA = enum.auto()


class BASE_MARKER: # Base marker class that OBJECT_MARKER and ARENA_MARKER derive from.
# TODO (2025): Do these colours need to change?
    team_marker_colors: dict = { # Colour definitions for each team defined in TEAMS
        TEAM.TEAM1: (255, 64, 0), # RED
        TEAM.TEAM2: (255, 255, 32), # YELLOW
        TEAM.TEAM3: (50,255,0), # GREEN
        TEAM.TEAM4: (255, 32, 255), # PURPLE
    }
...
# TODO (2025): Update colours accordingly
    @property
...
class OBJECT_MARKER(BASE_MARKER): # This is a game object rather than a wall. Add properties you want to keep track of
    def __init__(
        self, id: int, owner: TEAM
    ) -> None:
        super().__init__(id, MARKER_TYPE.GAME_OBJECT)
        self.owning_team = owner

    def __repr__(self) -> str:
        return f"<Marker(OBJECT) owning_team={self.owning_team} />"
...
class MARKER(BASE_MARKER): # This is literally just how the code gets the different marker types.
    ...
    def by_id(id: int, team: typing.Union[TEAM, None] = None) -> BASE_MARKER: # team is currently unused, but it is referenced throughout the code. It is the team of the robot I believe (check this)
        ...
        wrappingId = id % 16 # Make sure that the ID range wraps after 20 values.
        if wrappingId<4: # Jewel (Has a team)
            owning_team = TEAM[f"T{wrappingId}"] # Set to the corresponding TEAM enum.
        else: # Sheep (Not specific to any team)
            owning_team = TEAM["ARENA"]

        return OBJECT_MARKER(id, owning_team)